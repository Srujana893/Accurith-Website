  ...
  return (
    <svg viewBox="0 0 64 64" aria-hidden="true" className={cn('h-8 w-8 shrink-0', className)}>
      <mask id={maskId} maskUnits="userSpaceOnUse" x="0" y="0" width="64" height="64">
        ...
        <path d="M12 33 27.5 48.5 58 18" fill="none" stroke="#000" strokeWidth="8" />
      </mask>
      <path d="M32 5 59 55H5Z" fill="currentColor" mask={`url(#${maskId})`} />
      {/* The check resolves to the verified node — the only accent ink. */}
      <circle cx="54.5" cy="21.5" r="5" className={mono ? 'fill-current' : 'fill-brand-blue'} />
    </svg>
  );
}

export function Wordmark({ theme = 'light', className }: { theme?: Theme; className?: string }) {
  return (
    ...
    >
      ...
      <span className="whitespace-nowrap">
        ...
        <span className="bg-brand-blue mr-[0.05em] -ml-[0.2em] inline-block h-[0.15em] w-[0.15em] rounded-full align-[0.66em]" />
        th
      </span>
    </span>
  );
...
export default function Logo({
  ...
  return (
    <span className={cn('inline-flex items-center gap-2.5', className)}>
      <LogoMark
        className={cn(theme === 'light' ? 'text-brand-navy' : 'text-white', markClassName)}
      />
      <span className="flex flex-col justify-center">
        ...
        {tagline && (
          <span
            aria-hidden="true"
            className={cn(
              'mt-1 text-[0.5rem] leading-none font-medium tracking-[0.28em] uppercase',
